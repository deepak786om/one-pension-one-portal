...
import { PENSIONER, PAYMENTS, GRIEVANCES, DLC_STATUS } from "../data/pensioner.js";
...
export default function Dashboard({ roleId, onLogout }) {
  ...
  const openTile = (key) => {
    if (isPensioner && getPensionerModule(key)) {
      setActive(key);
      return;
    }
  ...
  if (ActiveModule) {
    return (
      <AnimatePresence mode="wait">
        <ActiveModule key={active} onBack={() => setActive(null)} />
      </AnimatePresence>
    );
  }
    ...
    <Screen className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-5">
        ...
        <div className="flex items-center gap-2.5">
          {isPensioner && (
            <Button variant="ghost" onClick={() => setActive("profile")} className="px-4 py-2.5">
              <Icon name="userCheck" size={16} /> My Profile
            </Button>
          )}
        ...
        {role.modules.map((m) => {
          ...
          return (
            <motion.button
              ...
              className="group relative flex flex-col items-start rounded-xl2 border border-border bg-card p-5 text-left shadow-card transition-shadow hover:shadow-elegant"
            >
